/*
AD4111.h
Include this file to use one AD4111 (singleton instance) connected to your MCU.
...
// Definitions, types and constants specific to the AD4111.
namespace AD4111 
{
	//Definitions for input channel selection bits for AD4111.
...
// Region with singleton pattern implementation -------------------------------
	private:
		...
		AD4111_Device() : AD411x_Device() {}

	public:
		// Aquire reference to the one and only instance, which is statically initialized.
		static AD4111_Device &getInstance()
		{
			static AD4111_Device instance; // Guaranteed to be destroyed. Instantiated on first use.
			return instance;
		}
		...
		AD4111_Device(AD4111_Device const&)   = delete;
		void operator=(AD4111_Device const&)  = delete;
	// ------------------------------------------------------------------------
#pragma endregion

	public:
		void configure_channel(byte channel, AD4111::Input input, byte setup, bool enable = true);

		bool check_id();
};

// Singleton instance declaration.
extern AD4111_Device &ad4111;
